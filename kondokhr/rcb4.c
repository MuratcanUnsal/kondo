 ...
 * interface: ftdi.h icinde tanımlanmıstır. Asagidakilerden biri secilebilir.
 ...
 * Basarılıysa 0, degilse <0 dondurur.
 */
...
/*-----------------------------------------------------------------------------
 * Swap dizisinden robota "n" adet "byte" gonderilir.
 * Basariliysa gonderilmis olan byte sayısını "n" dondurur. Hata varsa <0
 */
...
/*-----------------------------------------------------------------------------
 * İşlem şablonu: Temizle, robota out_bytes gonder, in_bytes al.
 * Hata varsa <0 doner
...
/*-----------------------------------------------------------------------------
 * Verilen hareket_id sine gore hareketi oynatır.
...
int kondo_play_motion(KondoRef ki, UINT num, long timeout)
{
	...
	// 4. Bitip bitmediğini her 50 saniyede bir kontrl et.

	// (1) Calismakta olan EEPROM programını durdur. ----------------------------------------
	// Bunu yapmak icin EEPROM kapatıp PGC yi hafızaya yazmanınız gerekir
	...
	ki->opt[0] |= RCB4_OPT_SIO; // servoların calistigindan emin l
	...
	// (4) 4. Bitip bitmediğini her 50 saniyede bir kontrl et. ------------------
...
/*-----------------------------------------------------------------------------
 * Bitfield icinde secilmis olan tum servolara ICS posizyon frame i gonder.
 * servos[5]: kullanılacak olan tum servolar icin bitfield.
 * frame: 2 byte lık gonderilecek olan ics frame i
 * 	serbest pozisyon  0x8000 (32768)
 *  tutunma pozisyonu  0x7fff (32767)
 *  merkez pozisyonu 0x1d4c (7500)
 *	Basarılıysa 0, hata varsa <0 dondurur.
 */
...
int kondo_send_ics_pos(KondoRef ki, UCHAR servos[5], UINT frame)
{
	...
	// ICS frame i gondermek icin komut yap.
	...
	// her bir servo icin frame hazırla
	for (i = 8; i < 8 + (c * 2); i += 2) {
...
int kondo_set_servo_pos(KondoRef ki, UINT servo_id, UINT frame)
{
	...
	//Tek bir servo yu kontrol etmek icin kullanılır.
	//Istenen servo_id ye gore, kondo_send_ics_pos gitmek uzere istenen servos
	//bitfield ini hazırlar ve gonderir.
